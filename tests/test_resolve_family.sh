...
chk "codex present → codex" "codex	false	do the thing" "$(PATH="$BOTH:$PATH" bash "$R" "do the thing")"
chk "codex absent → claude" "claude	false	do the thing" "$(PATH="$ONLYCLAUDE:/usr/bin:/bin" bash "$R" "do the thing")"
chk "override claude (codex present)" "claude	false	fix bug" "$(PATH="$BOTH:$PATH" bash "$R" "-- --reviewer claude -- fix bug")"
chk "override codex, codex absent → error" "error" "$(PATH="$ONLYCLAUDE:/usr/bin:/bin" bash "$R" "--reviewer codex -- x" 2>&1; echo)"
chk "task after -- preserved incl leading dashes" "claude	false	--do --not --strip" "$(PATH="$ONLYCLAUDE:/usr/bin:/bin" bash "$R" "-- --do --not --strip")"
...
chk "internal whitespace/tabs preserved" "$(printf 'claude\tfalse\t%s' "$TASK_WS")" \
...
chk "cmd-substitution task not executed" "$(printf 'claude\tfalse\t%s' "$TASK_CMDSUB")" \
...
chk "backtick task not executed" "$(printf 'claude\tfalse\t%s' "$TASK_BACKTICK")" \
...
chk "include-dirty before reviewer" "claude	true	fix bug" \
  "$(PATH="$BOTH:$PATH" bash "$R" "--include-dirty --reviewer claude -- fix bug")"
chk "include-dirty after reviewer" "claude	true	fix bug" \
  "$(PATH="$BOTH:$PATH" bash "$R" "--reviewer claude --include-dirty -- fix bug")"
chk "include-dirty without override" "codex	true	fix bug" \
